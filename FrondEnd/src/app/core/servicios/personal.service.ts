...
export class PersonalService {
  ...
  crear(personal: Personal, foto?: File): Observable<Personal> {
    ...
    formData.append(
      'personal',
      new Blob([JSON.stringify(personal)], { type: 'application/json' })
    );
    if (foto) {
  ...
  modificar(
    ...
    return this.http.put<Personal>(`${this.apiURL}/${codp}`, formData, {
      headers: this.getAuthHeaders(),
    });
  }

  eliminar(codp: number): Observable<void> {
    return this.http.delete<void>(`${this.apiURL}/${codp}`, {
      headers: this.getAuthHeaders(),
    });
  }
  ...
  registrarEnDatos(codp: number, cedula: string): Observable<any> {
    ...
    return this.http.post(
      `${environment.apiURL}/api/datos?${params.toString()}`,
      {},
      {
        headers: this.getAuthHeaders(),
      }
