...
export class GestionUsuariosComponent implements OnInit {
      ...
      next: (personaGuardada) => {
        if (
          ...
          this.personalService
            .registrarEnDatos(personaGuardada.codp!, this.nuevaPersona.cedula)
            .subscribe({
              next: () => {
              ...
              error: (err) => {
                console.error('Error al guardar en DATOS:', err.error.mensaje);
                this.manejarError(
                  err,
                  'Error al guardar datos complementarios'
                );
              },
            });
        } else {
  ...
  ejecutarConfirmacion(): void {
    ...
    obs.subscribe({
      next: () => {
        const accion =
          this.tipoConfirmacion === 'eliminar' ? 'eliminada' : 'habilitada';
    ...
    const filas = usuarios
      ...
      <tr>
        <td style="padding:12px; vertical-align:top;">
          <img src="${this.getFotoUrl(u.foto)}" alt="Foto de ${
          u.nombre
        }" style="width:72px;height:72px;object-fit:cover;border-radius:12px;border:1px solid #cbd5e1;" />
        </td>
        <td style="padding:12px;">
          <div style="font-weight:700;color:#0f172a;font-size:16px;">${u.ap} ${
          u.am ?? ''
        } ${u.nombre}</div>
          <div style="margin-top:4px;color:#1e293b;font-size:14px;">Cédula: <strong>${
            u.cedula ?? '—'
          }</strong></div>
          <div style="color:#334155;font-size:14px;">Tipo: ${
            u.tipo === 'P'
              ? 'Profesor'
              : u.tipo === 'E'
              ? 'Estudiante'
          ...
          <div style="color:#334155;font-size:14px;">Estado: ${
  ...
  guardarAcceso(): void {
    ...
    if (this.modoAccesoEdicion) {
      ...
      this.personalService.modificarUsuario(usuario.login, usuario).subscribe({
        next: () => {
          this.notificationService.showSuccess(
            'Acceso modificado correctamente'
          );
          this.cargarUsuarios();
          this.modalAccesoVisible = false;
        },
